...
namespace DevExtreme.AspNet.Data.Helpers {
    ...
    //            loadOptions.Sort = (JsonSerializer.Deserialize<SortingInfo[]>(sort)) ?? Array.Empty<SortingInfo>();

    //        if(!String.IsNullOrEmpty(group))
    //            loadOptions.Group = JsonSerializer.Deserialize<GroupingInfo[]>(group) ?? Array.Empty<GroupingInfo>();

    //        if (!String.IsNullOrEmpty(filter))
    //        {
    //            loadOptions.Filter = JsonSerializer.Deserialize<IList>(filter) ?? Array.Empty<SortingInfo>();
    //        }

    //        if(!String.IsNullOrEmpty(totalSummary))
    //            loadOptions.TotalSummary = JsonSerializer.Deserialize<SummaryInfo[]>(totalSummary) ?? Array.Empty<SummaryInfo>();

    //        if(!String.IsNullOrEmpty(groupSummary))
    //            loadOptions.GroupSummary = JsonSerializer.Deserialize<SummaryInfo[]>(groupSummary) ?? Array.Empty<SummaryInfo>();
